...
PREDICTIONS_CSV = Path(f"experiments/E001_D1_resnet18_baseline/{SEED_SUBDIR}/d3c_predictions.csv")
TEMP_METRICS_JSON = Path(f"experiments/E001_D1_resnet18_baseline/{SEED_SUBDIR}/temperature_scaling_metrics.json")

OUT_PREDICTIONS_CSV = Path(f"experiments/E001_D1_resnet18_baseline/{SEED_SUBDIR}/d3c_temperature_scaled_predictions.csv")
OUT_METRICS_JSON = Path(f"experiments/E001_D1_resnet18_baseline/{SEED_SUBDIR}/d3c_temperature_scaled_metrics.json")
REPORT_PATH = Path("reports/experiments/E001_D3C_temperature_scaled_results.md")
...
def write_report(temperature, raw_metrics, scaled_metrics):
    ...
    with REPORT_PATH.open("w", encoding="utf-8") as f:
        f.write("# E001 on D3C — Temperature-Scaled Domain-Shift Evaluation\n\n")

        f.write("## Experiment\n\n")
        f.write(
            "E001 ResNet18 trained on D1, evaluated on D3C UPENN-GBM central slices. "
            "The temperature value was learned previously using the D1 validation set only.\n\n"
        )

        f.write("## Inputs\n\n")
        f.write(f"- Raw D3C predictions: `{PREDICTIONS_CSV}`\n")
        ...
        f.write(
            "D3C is glioma-focused and does not contain the full four-class label set. "
        ...
        f.write(
            ...
            "feature representation. If the prediction distribution remains unstable across D3C, this indicates that "
...
def main():
    ...
    print("D3C temperature-scaled evaluation complete.")
