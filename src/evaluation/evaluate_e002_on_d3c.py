...
EXPERIMENT_DIR = Path(f"experiments/E002_D1_efficientnet_b0_baseline/{SEED_SUBDIR}")
...
REPORT_PATH = Path("reports/experiments/E002_D3C_domain_shift_results.md")
...
def write_report(metrics, pred_df):
    ...
    with REPORT_PATH.open("w", encoding="utf-8") as f:
        f.write("# E002 on D3C Domain-Shift Evaluation\n\n")

        f.write("## Experiment\n\n")
        f.write("E002 EfficientNet-B0 trained on D1 leakage-aware split, evaluated on D3C UPENN-GBM central slices.\n\n")

        f.write("## Inputs\n\n")
        f.write(f"- Checkpoint: `{CHECKPOINT_PATH}`\n")
        f.write(f"- D3C manifest: `{D3C_MANIFEST}`\n")
        f.write(f"- D3C slices evaluated: {metrics['n_slices']}\n")
        ...
        f.write(
            "D3C is glioma-focused and does not contain the full D1 four-class label set. "
        ...
        f.write("\n## Interpretation\n\n")
        f.write(
            "This report evaluates whether the D1-trained E002 model recognises D3C glioma-domain images "
            "as glioma and how confident it is under domain shift. Because D3C labels are collection-level "
        ...
        f.write(
            "The next step is to apply the E002 temperature scaling parameter to these D3C logits/probabilities "
...
def main():
    ...
    dataset = D3CSliceDataset(D3C_MANIFEST, transform=build_transform())
    loader = DataLoader(
        dataset,
