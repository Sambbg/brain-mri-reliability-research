...
REPORT_PATH = Path("reports/datasets/D1_D3C_exact_overlap_report.md")
OUT_CSV = Path("reports/datasets/D1_D3C_exact_overlap_pairs.csv")
...
def main():
    ...
    if not D3C_MANIFEST.exists():
        raise FileNotFoundError(f"Missing D3C manifest: {D3C_MANIFEST}")
    ...
    if "sha256_recomputed" not in d3c.columns:
        raise ValueError("D3C manifest missing sha256_recomputed column.")

    d1_hashes = set(d1["sha256"].dropna().astype(str))
    d3c_hashes = set(d3c["sha256_recomputed"].dropna().astype(str))

    shared_hashes = sorted(d1_hashes.intersection(d3c_hashes))

    rows = []

    if shared_hashes:
        d1_shared = d1[d1["sha256"].astype(str).isin(shared_hashes)].copy()
        d3c_shared = d3c[d3c["sha256_recomputed"].astype(str).isin(shared_hashes)].copy()

        for _, d1_row in d1_shared.iterrows():
            h = str(d1_row["sha256"])
            matches = d3c_shared[d3c_shared["sha256_recomputed"].astype(str) == h]

            for _, d3c_row in matches.iterrows():
                rows.append({
                    "sha256": h,
    ...
    with REPORT_PATH.open("w", encoding="utf-8") as f:
        f.write("# D1-D3C Exact Overlap Report\n\n")

        f.write("## Input\n\n")
        f.write(f"- D1 manifest: `{D1_MANIFEST}`\n")
        f.write(f"- D3C manifest: `{D3C_MANIFEST}`\n")
        f.write(f"- D1 rows: {len(d1)}\n")
        f.write(f"- D3C rows: {len(d3c)}\n\n")
        ...
        if len(out_df) == 0:
            f.write(
                ...
                "and D3C converted central-slice PNG images. This is expected because "
                "D3C images were generated from DICOM conversion and are unlikely to be "
            ...
            f.write(
                ...
                "investigated before using D3C for evaluation.\n"
            )
        ...
        if len(out_df) > 0:
            ...
            for idx, row in out_df.head(20).iterrows():
                ...
                f.write(f"- D3C: `{row['d3c_patient_id']}` / `{row['d3c_series_description']}` / `{row['d3c_output_image_path']}`\n\n")

    print(f"D1 rows: {len(d1)}")
    print(f"D3C rows: {len(d3c)}")
