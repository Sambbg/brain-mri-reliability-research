...
IN_PATH = Path("data/processed/D1_manifest.csv")
OUT_PATH = Path("data/processed/D1_manifest_deduplicated.csv")
REPORT_PATH = Path("reports/datasets/D1_deduplication_report.md")
...
def main():
    ...
    for sha, items in sorted(by_hash.items()):
        # Deterministic rule:
        # keep the first item after sorting by split, class, filepath.
        sorted_items = sorted(
            items,
            key=lambda r: (r["split"], r["class_label"], r["filepath"])
        )
    ...
    with REPORT_PATH.open("w", encoding="utf-8") as f:
        f.write("# D1 Deduplication Report\n\n")
        ...
        f.write(
            ...
            "one row was retained using a deterministic sort by split, class label, and filepath. "
        ...
        f.write("\n## Interpretation\n\n")
        f.write(
            "The deduplicated manifest should be preferred for training and evaluation. "
            "The original manifest is retained for auditability. Exact duplicate removal reduces repeated-image bias, "
            "but it does not remove near-duplicates or same-patient adjacent slices.\n"
        )
