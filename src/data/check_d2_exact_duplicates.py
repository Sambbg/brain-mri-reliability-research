...
MANIFEST_PATH = Path("data/processed/D2_manifest.csv")
OUT_PATH = Path("reports/datasets/D2_exact_duplicate_report.md")
...
def main():
    ...
    for sha, items in duplicate_groups.items():
        splits = {item["source_split"] for item in items}
    ...
    with OUT_PATH.open("w", encoding="utf-8") as f:
        f.write("# D2 Exact Duplicate Report\n\n")
        f.write("## Input\n\n")
        ...
        if duplicate_group_count == 0:
            f.write(
                "No exact SHA256 duplicates were found within D2. "
                "This reduces one internal leakage concern, but it does not rule out "
                "near-duplicates, same-patient adjacent slices, or overlap with D1.\n\n"
            )
        else:
            f.write(
                "Exact duplicate SHA256 hashes were found within D2. These must be investigated "
                "before using D2 for external evaluation. Cross-split duplicates are especially serious.\n\n"
            )
        ...
        if cross_split_groups:
            ...
            for idx, (sha, items) in enumerate(cross_split_groups.items(), start=1):
                ...
                for item in items:
                    f.write(
                        f"- `{item['source_split']}` / `{item['class_label']}` / `{item['filepath']}`\n"
                    )
                f.write("\n")
