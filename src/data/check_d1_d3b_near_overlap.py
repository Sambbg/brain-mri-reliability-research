...
REPORT_PATH = Path("reports/datasets/D1_D3B_near_overlap_report.md")
OUT_CSV = Path("reports/datasets/D1_D3B_near_overlap_pairs.csv")
...
def main():
    ...
    if not D3B_MANIFEST.exists():
        raise FileNotFoundError(f"Missing D3B pHash manifest: {D3B_MANIFEST}")
    ...
    if "phash" not in d3b.columns:
        raise ValueError("D3B manifest missing phash column.")

    d1_rows = d1[d1["phash"].notna() & (d1["phash"].astype(str).str.len() > 0)].copy()
    d3b_rows = d3b[d3b["phash"].notna() & (d3b["phash"].astype(str).str.len() > 0)].copy()

    pairs = []
    distance_counts = Counter()

    total_comparisons = len(d1_rows) * len(d3b_rows)

    print(f"D1 rows: {len(d1_rows)}")
    print(f"D3B rows: {len(d3b_rows)}")
    print(f"Total comparisons: {total_comparisons}")

    d3b_records = list(d3b_rows.to_dict("records"))
    ...
    for i, d1_row in enumerate(d1_rows.to_dict("records")):
        ...
        for d3b_row in d3b_records:
            d3b_phash = d3b_row.get("phash", "")
            dist = hamming_distance_hex(d1_phash, d3b_phash)
            ...
            if dist <= PHASH_THRESHOLD:
                ...
                pairs.append({
                    ...
                    "d1_phash": d1_phash,
                    "d3b_dataset_id": d3b_row.get("dataset_id", ""),
                    ...
                    "d3b_slice_index_in_sorted_series": d3b_row.get("slice_index_in_sorted_series", ""),
                    "d3b_output_image_path": d3b_row.get("output_image_path", ""),
                    "d3b_label": d3b_row.get("label", ""),
                    "d3b_phash": d3b_phash,
                    "cross_class_pair": str(d1_row.get("class_label", "")) != str(d3b_row.get("label", "")),
                })
    ...
    if not out_df.empty:
        out_df = out_df.sort_values(
            by=["phash_distance", "d1_class_label", "d3b_patient_id", "d1_filepath"],
    ...
    with REPORT_PATH.open("w", encoding="utf-8") as f:
        f.write("# D1-D3B pHash Near-Overlap Report\n\n")

        f.write("## Input\n\n")
        f.write(f"- D1 pHash manifest: `{D1_MANIFEST}`\n")
        f.write(f"- D3B pHash manifest: `{D3B_MANIFEST}`\n")
        f.write(f"- D1 rows compared: {len(d1_rows)}\n")
        f.write(f"- D3B rows compared: {len(d3b_rows)}\n")
        ...
        if len(out_df) == 0:
            f.write(
                "No pHash near-overlap was detected between D1 and D3B at the selected threshold. "
                "This supports treating D3B as visually distinct from D1 for the planned domain-shift "
                "confidence analysis, although pHash cannot prove patient-level independence.\n"
            )
        else:
            f.write(
                "pHash near-overlap was detected between D1 and D3B. These pairs may represent visual similarity, "
        ...
        if not out_df.empty:
            ...
            for idx, row in out_df.head(50).iterrows():
                ...
                f.write(
                    f"- D3B: `{row['d3b_patient_id']}` / `{row['d3b_series_description']}` / "
                    f"`{row['d3b_output_image_path']}`\n"
                )
                f.write(f"- Cross-class pair: `{row['cross_class_pair']}`\n\n")

    print(f"D1 rows compared: {len(d1_rows)}")
    print(f"D3B rows compared: {len(d3b_rows)}")
    print(f"Total comparisons: {total_comparisons}")
