...
DATASET_ID = "D2_brisc2025"
RAW_ROOT = Path("data/raw/D2_brisc2025/brisc2025/classification_task")
OUT_PATH = Path("data/processed/D2_manifest.csv")
...
def main():
    ...
    for split_dir in ["train", "test"]:
        ...
        for class_dir in sorted([p for p in split_path.iterdir() if p.is_dir()]):
            raw_class_label = class_dir.name

            if raw_class_label not in CLASS_MAP:
                raise ValueError(f"Unexpected D2 class folder: {raw_class_label}")

            mapped_class_label = CLASS_MAP[raw_class_label]
            ...
            for image_path in sorted(class_dir.rglob("*")):
                ...
                try:
                    ...
                    rows.append({
                        ...
                        "class_label": mapped_class_label,
    ...
    for row in rows:
        split_counts[row["source_split"]] = split_counts.get(row["source_split"], 0) + 1
        key = (row["source_split"], row["class_label"])
